function tc = TwoHeat_Tc_V4_new(D2s,a,Nu,Tc,g)
...
sol = ode23s(@(t,y) heat2(t,y,dx1,D1,D2,M,N,NU,G),[0 tend], IC,options);
...
function dy = heat2(~,y,dx1,D1,D2,M,N,NU,G)
dy=zeros(N,1);
...
dy(M) = 1/dx1^2*(D2*(-1.5*y(M)+2*y(M+1)-0.5*y(M+2))-D1*(0.5*y(M-2)-2*y(M-1)+1.5*y(M)));
 ...
 for i=(M+1):(N-1)
     dy(i) = D2/dx1^2 * (y(i+1)-2*y(i)+y(i-1))-G*(y(i)) ;
 end
